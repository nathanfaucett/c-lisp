#ifndef __LISP_CORE_MEMORY_C__
#define __LISP_CORE_MEMORY_C__
...
static void lisp_MemoryNode_destructor(lisp_MemoryNode* node) {
    if (node->next != NULL) {
...
static lisp_Memory* lisp_Memory_constructor(lisp_Memory* memory) {
    memory->root = NULL;
    memory->tail = NULL;
    memory->size = 0;
    return memory;
...
static void lisp_Memory_destructor(lisp_Memory* memory) {
    if (memory->root != NULL) {
...
static void lisp_Memory_delete(lisp_Memory* memory) {
    lisp_Memory_destructor(memory);
    free(memory);
}

static lisp_Value* lisp_Memory_alloc(lisp_Memory* memory) {
...
static void lisp_Memory_dealloc(lisp_Memory* memory, lisp_Value* value) {

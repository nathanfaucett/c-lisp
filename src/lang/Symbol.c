...
inline static void lisp_Symbol_destructor(lisp_State* state, lisp_Symbol* symbol) {
    lisp_Value_deref(state, symbol->string);
}

inline static struct lisp_Symbol* lisp_Symbol_from_cstring(lisp_State* state, lisp_Symbol* symbol, lisp_u8* cstring) {

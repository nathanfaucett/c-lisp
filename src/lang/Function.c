...
static lisp_Value* lisp_Function_call(lisp_State* state, lisp_Function* fn, lisp_Value* args_value, lisp_Scope* scope) {
    if (fn->native != NULL) {
        ...
        lisp_Vector* params = (lisp_Vector*) fn->params->value;
        ...
        for (lisp_u64 i = 0, il = params->size; i < il; i++) {
            ...
            if (rest) {
                ...
                if (lisp_cstring_equal(cstring, "...")) {
                    ...
                    lisp_Scope_def(fn_scope, param, lisp_State_eval(state, lisp_List_get(state, args, i), scope));
                }
